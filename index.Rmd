...
autoplot(stl2) + geom_line(color="blue") + ggtitle("Seasonal decompostion of visitors") + xlab("Number of weeks from the begining")
```
...
autoplot(window(stl$time.series[,1], end=5)) + ggtitle("Weekly seasonality") + ylab("Number of Visitors") + xlab("Number of weeks from the begining")
```
...
autoplot(forecast(stl, h=120),  include = 150, plot.conf = F) + ggtitle("Forecast using STL") + ylab("Number of visitors") +
  geom_abline(slope = 0, intercept = av, color="magenta") + xlab("Number of weeks from the begining") + 
  scale_colour_manual("", values = c("blue", "red"), labels = c("observations", "my forecast"))
```
